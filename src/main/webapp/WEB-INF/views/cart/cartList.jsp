	...
	<c:choose>
		<c:when test="${list != '[]'  }">
			...
			<div class="cartInfo">
				...
				<input type="checkbox" name="buy" value="${i.g_num }" class="check" checked="checekd"><br>
				<input type="hidden" class="individual_price" value="${i.g_price }">
				<input type="hidden" name="buystock" class="individual_cartstock" value="${i.cartstock}">
				<input type="hidden" class="individual_point" value="${i.point}">
			</div>	
...
${i.g_option }<br>
			...
			<hr>
<div >총 수량<span class="totalCount"></span></div> 
<div >배송비<span class="deliveryPrice"></span></div> 
<div >적립 포인트<span class="totalPoint"></span></div>  
<div >상품 금액<span class="totalPrice"></span></div> 
<div >주문 금액<span class="finalTotalPrice"></span></div>  
...
장바구니가 비었습니다
	...
	function setTotalInfo(){
		...
		$(".cartInfo").each(function(index, element){
			...
			// 총 갯수
			totalCount += parseInt($(element).find(".individual_cartstock").val());
			// 총 가격
			totalPrice += (parseInt($(element).find(".individual_price").val()*parseInt($(element).find(".individual_cartstock").val())));
			// 총 마일리지
			totalPoint += parseInt($(element).find(".individual_point").val());			
...
$(".modi").on("click",function(){
	let stock= $($($(this).siblings()[0]).children()[1]).text();
	console.log(stock);
	location.href="/cart/cartModi?g_num="+$(this).val()+"&cartstock="+stock;
});
